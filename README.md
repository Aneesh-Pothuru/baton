...
It can also switch to **Installed live service** when served by the local
control plane, reading real durable runs and events without replacing the
offline fixture.
...
BATON now includes a real local HTTP control plane backed by the same SQLite
runtime:
...
Visit `http://127.0.0.1:8020/demo/`, choose **Installed live service**, then
register/fire routines and inspect the score as API events arrive. The service
supports routine registration and firing, durable runs, claim acquire/renew/
release, scoped advisories, objective updates, budget signals, checkpoints,
resume, evidence reads, health, and readiness. State survives service restart.

The server binds to loopback by default. A non-loopback bind is rejected unless
`BATON_API_TOKEN` is configured. Docker Compose additionally publishes only to
host loopback, drops Linux capabilities, uses a read-only root filesystem, and
persists SQLite in a named volume:

```bash
cp .env.example .env
# Replace the placeholder with: python -c 'import secrets; print(secrets.token_urlsafe(32))'
docker compose up --build
```

The complete endpoint contract and curl journey are in
